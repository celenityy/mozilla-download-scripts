...
LATEST_VERSION=$("${CURL}" ${CURL_FLAGS} -s "${THUNDERBIRD_URL}" | "${GREP}" -oP 'https://download-installer\.cdn\.mozilla\.net/pub/thunderbird/releases/\K[0-9]+\.[0-9]+(\.[0-9]+)?')
...
"${CURL}" ${CURL_FLAGS} --tlsv1.3 -O "${THUNDERBIRD_DOWNLOAD_URL}"
...
"${ROOT}" "${CHMOD}" -R -v 644 "${THUNDERBIRD_DIR}"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/chrome"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/chrome/icons"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/chrome/icons/default"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/defaults"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/defaults/messenger"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/defaults/pref"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/fonts"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/glxtest"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/icons"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/isp"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/rnp-cli"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/rnpkeys"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/thunderbird"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/thunderbird-bin"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/updater"
"${ROOT}" "${CHMOD}" -v 755 "${THUNDERBIRD_DIR}/vaapitest"
...
if [[ -f "${HOME}/.local/bin/thunderbird" ]]; then
    rm -f "${HOME}/.local/bin/thunderbird"
fi
"${LN}" -s "${THUNDERBIRD_DIR}/thunderbird" "${HOME}/.local/bin/thunderbird"
